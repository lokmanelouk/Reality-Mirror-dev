    ...
    <div className="flex flex-col items-center gap-4 py-6 px-4 border border-white/5 bg-white/[0.01] relative overflow-hidden group">
      <div className="relative">
        <div className={`transition-all duration-1000 ${phase === 'loading' ? 'rotate-180 scale-100 opacity-100' : 'rotate-0 scale-75 opacity-20'}`}>
          <RefreshCw size={48} className="text-red-950 animate-spin transition-all duration-1000" style={{ animationDuration: '3s' }} />
        </div>
      ...
      <div className="h-[1px] w-full bg-slate-900 overflow-hidden relative">
         <div className={`absolute inset-0 bg-red-600 transition-transform duration-[4000ms] ease-linear ${phase === 'loading' ? 'translate-x-[-100%]' : 'translate-x-0'}`}></div>
      </div>
...
export const Onboarding: React.FC<Props> = ({ onComplete }) => {
  ...
  const getIntensityInfo = () => {
    if (hours < 4) return { label: 'LOW_INTENSITY', icon: <Activity size={14} className="text-slate-700" />, desc: 'Insufficient drive. Likely to be forgotten by history.', color: 'text-slate-700', bg: 'bg-slate-900/10' };
    if (hours < 7) return { label: 'SUSTAINABLE', icon: <Target size={14} className="text-blue-500" />, desc: 'Human-grade effort. Safe, but perhaps too safe.', color: 'text-blue-500', bg: 'bg-blue-500/10' };
    if (hours < 10) return { label: 'HIGH_PRIORITY', icon: <Zap size={14} className="text-orange-500" />, desc: 'Ambitious load. Social life decay likely.', color: 'text-orange-500', bg: 'bg-orange-500/10' };
    return { label: 'TOTAL_DEVOTION', icon: <Flame size={14} className="text-red-600 animate-pulse" />, desc: 'System critical load. Madness or mastery awaits.', color: 'text-red-600', bg: 'bg-red-600/10' };
  };
          ...
          <div className="font-mono text-[8px] space-y-3 text-slate-700 overflow-hidden flex-1 relative z-10">
            <div className="animate-logs-scroll space-y-4">
              {Array.from({length: 30}).map((_, i) => (
                <div key={i} className={`flex flex-col gap-1 transition-opacity duration-1000 ${i % 3 === 0 ? 'animate-terminal-flicker text-red-900/50' : ''}`}>
            ...
            {step === 2 && (
              <div className="animate-in fade-in slide-in-from-bottom-6 duration-700">
                ...
                <div className="flex flex-col gap-10">
                   ...
                   <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start max-w-4xl">
                      {/* Strategic Quota Module */}
                      <div className="relative p-6 md:p-8 bg-[#050505] border border-white/5 overflow-hidden group shadow-2xl">
                         <div className="absolute top-0 left-0 w-6 h-6 border-t border-l border-red-600/50 group-focus-within:border-red-600 transition-colors"></div>
                         <div className="absolute bottom-0 right-0 w-6 h-6 border-b border-r border-red-600/50 group-focus-within:border-red-600 transition-colors"></div>
                         ...
                         <div className="relative z-10 flex flex-col gap-8">
                            <div className="flex justify-between items-center text-[8px] font-mono text-slate-700 uppercase tracking-tighter">
                               <span className="flex items-center gap-2"><Scale size={10} /> EXERTION_SLIDER</span>
                               <span className="px-2 py-0.5 border border-current">{info.label}</span>
                            </div>

                            <div className="space-y-4">
                               <div className="flex justify-between font-mono text-[9px] text-slate-800">
                                  <span>01_HR</span>
                                  <span>16_HR</span>
                               </div>
                               <input 
                                 ...
                                 max="16" 
                                 step="0.5" 
                                 className="w-full cursor-crosshair accent-red-600 h-1 bg-slate-950 rounded-none" 
                                 value={hours} 
                                 onChange={e => setHours(Number(e.target.value))} 
                               />
                            </div>

                            <div className="p-4 bg-black/40 border border-white/5 flex items-start gap-4">
                               <AlertCircle size={14} className={`${info.color} shrink-0 mt-0.5`} />
                               <p className="text-[9px] font-mono text-slate-500 uppercase leading-relaxed font-bold italic">{info.desc}</p>
                            </div>
                         </div>
                      </div>

                      {/* Display readout */}
                      <div className="h-full flex flex-col justify-between p-8 border border-white/5 bg-white/[0.01] relative overflow-hidden group">
                         <div className="absolute top-0 right-0 p-2 font-mono text-[6px] text-slate-900 opacity-20 group-hover:opacity-100 transition-opacity">
                            HEX_READOUT: 0x{hours.toString(16).toUpperCase()}
                         </div>
                         <div className="text-right">
                            <span className={`text-8xl md:text-9xl font-black leading-none tracking-tighter transition-all duration-500 ${hours >= 10 ? 'text-red-600 vibration-active' : 'text-white'}`}>
                               {hours}
                            </span>
                            <span className="text-slate-800 text-3xl font-black ml-2 font-mono">H</span>
                         </div>
                         <div className="flex justify-between border-t border-slate-900 pt-6 mt-6">
                            <div className="flex flex-col">
                               <span className="text-[7px] font-mono text-slate-800 uppercase">INTENSITY_TAG</span>
                               <span className={`text-[10px] font-black uppercase tracking-widest ${info.color}`}>{info.label}</span>
                            </div>
                            <div className="flex flex-col text-right">
                               <span className="text-[7px] font-mono text-slate-800 uppercase">LOAD_LEVEL</span>
                               <span className="text-[10px] font-black uppercase tracking-widest text-slate-200">
                                  {hours > 12 ? 'CRITICAL' : hours > 8 ? 'STABLE' : 'NOMINAL'}
                               </span>
                            </div>
                         </div>
                      </div>
